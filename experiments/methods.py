...
def _run_clustering_generic(sim_data_matrix, k, clustering_algorithm):
    ...
    if clustering_algorithm == 'spectral':
        return SpectralClustering(n_clusters=k, affinity='precomputed').fit_predict(sim_data_matrix)

    elif clustering_algorithm.startswith('hc-'):
...
def run_pearson(data, k, n_jobs=1, **kwargs):
    ...
    return _run_clustering_generic(r_sim_mat, k, **kwargs)
